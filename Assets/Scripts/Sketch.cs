        ...
        class SketchRenderPass : ScriptableRenderPass
        {
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                using (new ProfilingScope(cmd, profilingSampler))
                {
                    ...
                    cmd.Blit(shadowmapTexture, shadowmapHandle1.rt);
            ...
            {
                var settings =
                    VolumeManager.instance.stack.GetComponent<SketchSettings>();

                Material material = data.material;

                material.SetTexture(
                    "_SketchTexture",
                    settings.sketchTexture.value
                );

                material.SetColor(
                    "_SketchColor",
                    settings.sketchColor.value
                );

                material.SetVector(
                    "_SketchTiling",
                    settings.sketchTiling.value
                );

                material.SetVector(
                    "_SketchThresholds",
                    settings.sketchThresholds.value
                );

                material.SetFloat(
                    "_DepthSensitivity",
                    settings.extendDepthSensitivity.value
                );

                material.SetFloat(
                    "_CrossHatching",
                    settings.crossHatching.value ? 1 : 0
                );
                ...
                material.SetFloat(
                    "_Spread",
                    settings.blurAmount.value / 6.0f
                );

                material.SetInt(
                    "_BlurStepSize",
                    settings.blurStepSize.value
                );
            ...
            public override void RecordRenderGraph(RenderGraph renderGraph,ContextContainer frameData)
            {
                        ...
                        profilingSampler))
                {
                    ...
                    builder.SetRenderFunc(
                        (CopyPassData data, RasterGraphContext context) =>
                            ExecuteCopyPass(
                                context.cmd,
                                data.inputTexture
                            )
                ...
                // MAIN PASS
                using (var builder =
                    renderGraph.AddRasterRenderPass<MainPassData>(
                        "Sketch_MainPass",
                        out var passData,
                        profilingSampler))
                {
                    ...
                    builder.SetRenderFunc(
                        (MainPassData data, RasterGraphContext context) =>
